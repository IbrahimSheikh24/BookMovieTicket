...
const ShowPage = () => {
  ...
  const handleBookSeats = () => {
    ...
    fetch("http://localhost:5001/api/booking/get-payment-secret", {
      method: "POST",
      headers: { "Content-Type": "application/json", jwttoken: window.localStorage.getItem('token') },
      body: JSON.stringify({
        seats: selectedSeats,
  ...
  useEffect(() => {
    fetch(`http://localhost:5001/api/show/${showId}`, {
      headers: {
  ...
  useEffect(() => {
    ...
    if (transactionId) {
      fetch("http://localhost:5001/api/booking/confirm", {
        method: "POST",
        headers: { "Content-Type": "application/json", jwttoken: window.localStorage.getItem('token') },
        body: JSON.stringify({
